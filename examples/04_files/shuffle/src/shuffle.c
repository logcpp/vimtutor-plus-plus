...
Namelist* get_input();
char* buf_push(char* buf, int buf_len, char c);
char** namelist_push(char** namelist, int namelist_len, char* buf);
...
int main(int argc, char** argv)
{
	...
	// output
	for (int i = 0; i < nl->namelist_len; i++){
		printf("%s\n", nl->namelist[i]);
	}
...
char** namelist_push(char** namelist, int namelist_len, char* buf)
{
	...
	if (namelist != NULL){
		for (int i = 0; i < namelist_len-1; i++){
